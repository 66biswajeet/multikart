...
const ProductForm = ({ updateId, title, buttonName, saveButton, setSaveButton }) => {
  ...
  const { data: oldData, isLoading: oldDataLoading, refetch, status } = useCustomQuery([updateId], () => request({ url: `${product}/${updateId}` }, router), { refetchOnWindowFocus: false, enabled: !!updateId, select: (data) => data?.data });
  useEffect(() => {
  ...
  const watchEvent = useCallback(
    (oldData, updateId) => {
      ...
      const values = ProductInitValues(oldData?.data, updateId);
  ...
  return (
    ...
    >
      {({ values, setFieldValue, errors, touched, isSubmitting, setErrors, setTouched, isValid, dirty }) => {
