...
const VariationTop = ({ data, index, values, setFieldValue, errors }) => {
  ...
  const setVariationValue = (name, value, index) => {
    setFieldValue("combination",values["combination"]?.map((item, i) => (i == index ? { ...item, name: value, values: [] } : item)),);
  };
  ...
  return (
    <div className="variant-row">
        ...
        nameList={[
          {
            ...
            inputprops: {
              name: "attribute_name",
              id: "attribute_name",
              options: data?.data?.filter((item) => (values["combination"].find((elem) => elem?.name?.name == item.name) ? false : true)),
      ...
      />
      <MultiSelectField errors={errors} values={values["combination"][index]} setFieldValue={setAttributesValues} name="values" title="Value" data={values?.["combination"]?.[index]?.["name"]?.["attribute_values"]?.map((elem) => ({ name: elem.value, id: elem._id || elem.id }))} />
