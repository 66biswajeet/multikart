  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ToastNotification("success", result.message || `${entityName} deleted successfully`);
        ...
        ToastNotification("error", result.message || `Failed to delete ${entityName}`);
      }
    } catch (error) {
      console.error('Delete error:', error);
      ToastNotification("error", error.message || "An error occurred while deleting");
    } finally {
      setIsDeleting(false);
      setModal(false);
    }
