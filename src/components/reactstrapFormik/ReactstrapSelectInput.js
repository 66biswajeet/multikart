...
const ReactstrapSelectInput = ({
  ...
  const onSelectValue = (option) => {
    if (Array.isArray(field?.value)) {
      ...
      const index = temp.findIndex(
        (elem) => String(elem[getValuesKey]) === String(option[getValuesKey])
      );
      if (index !== -1) {
        temp.splice(index, 1);
      } else {
      ...
      const valueToSet =
        props.store === "obj"
          ? option
          : option[getValuesKey] || option._id || option.id;
  ...
  useEffect(() => {
    ...
    if (Array.isArray(field?.value)) {
      ...
      const filteredItems =
        sourceList?.filter((elem) =>
          field?.value?.includes(elem[getValuesKey])
        ) || [];
      setSelectedItems(filteredItems);
    } else {
      // Setting variables for type String data
      const sourceList = props.inputprops?.setsearch ? listOpt : list;
      const foundItem = sourceList?.find(
        (elem) => String(field?.value) === String(elem[getValuesKey])
      );
    ...
    <>
      ...
      <div
        className={`custom-select-box ${
          props.formfloat == "true" && "form-floating"
        }`}
        ref={ref}
      >
        {Array.isArray(selectedItems) ? (
          <div
            ...
            onClick={() =>
              setIsComponentVisible((p) => p !== field?.name && field?.name)
            }
          >
            <div className={`bootstrap-tagsinput form-select`}>
              {selectedItems.length > 0 ? (
                      ...
                      <RiCloseLine
                        onClick={(e) => {
                          ...
                          setSelectedItems((p) =>
                            p.filter(
                              (elem) =>
                                elem[getValuesKey] !== item[getValuesKey]
                            )
                          );
                          setFieldValue(
                            field.name,
                ...
                <span>
                  {t(
                    props?.inputprops?.initialTittle
                      ? props?.inputprops?.initialTittle
                      : "Select"
                  )}
                </span>
                ...
                setvalue !== undefined
                  ...
                  : selectedItems?.name ||
                      props.inputprops?.options?.find(
                        (item) =>
                          String(item[getValuesKey]) === String(field.value)
                      )?.name
              ) ||
              t(
                props?.inputprops?.initialTittle
