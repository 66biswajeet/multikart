...
const ReactstrapSelectInput = ({ field, form: { touched, errors, setFieldValue }, getValuesKey = "id", setvalue,noSearchBar, ...props }) => {
  ...
  const onSelectValue = (option) => {
    ...
    if (Array.isArray(field?.value)) {
      ...
      const valueToSet = props.store === "obj" ? option : (option[getValuesKey] || option._id || option.id);
      ...
      <div className={`custom-select-box ${props.formfloat=="true"&&'form-floating'}`} ref={ref}>
        {Array.isArray(selectedItems) ?
          ...
          <Input type="text" className="form-control form-select cursor position-absolute"
            value={t(setvalue !== undefined ? (props.inputprops.value || selectedItems?.name) : (selectedItems?.name || props.inputprops?.options?.find((item) => String(item[getValuesKey]) === String(field.value))?.name)) || t(props?.inputprops?.initialTittle ? props?.inputprops?.initialTittle :"Select")}
