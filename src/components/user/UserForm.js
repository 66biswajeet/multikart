...
import { emailSchema, nameSchema, passwordConfirmationSchema, passwordSchema, phoneSchema, roleIdSchema, YupObject } from "../../utils/validation/ValidationSchemas";
...
const UserForm = ({ updateId, fixedRole, noRoleField, addAddress, type, buttonName }) => {
  ...
  const { data: oldData, isLoading: oldDataLoading, refetch } = useCustomQuery([updateId], () => request({ url: `/user/${updateId}` }, router), { enabled: false, refetchOnMount: false });
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        role: updateId ? (oldData?.data?.data?.role?._id || oldData?.data?.data?.role?.id || oldData?.data?.data?.role) || "" : rolesData ? "" : "",
        status: updateId ? Boolean(Number(oldData?.data?.data?.status)) : true,
        address: [],
        country_code: updateId ? oldData?.data?.data?.country_code || "" : "91",
      }}
