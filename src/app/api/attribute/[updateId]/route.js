...
import Attribute from "@/models/Attributes"; // Corrected import from @/models/Attributes
import Category from "@/models/Category"; // Import Category to check for links
  ...
  try {
    ...
    const updatedAttribute = await Attribute.findByIdAndUpdate(
      updateId,
      {
        ...
        slug,
        status:
          body.status !== undefined ? body.status : existingAttribute.status,
...
export async function DELETE(request, { params }) {
  try {
    ...
    // **VALIDATION ADDED**: Check if attribute is in use by any category
    const categoryInUse = await Category.findOne({
      "attribute_mapping.attribute_id": updateId,
    });
    if (categoryInUse) {
      return NextResponse.json(
        {
          success: false,
          message:
            "Cannot delete attribute: It is linked to one or more categories.",
        },
        { status: 400 }
      );
    }
