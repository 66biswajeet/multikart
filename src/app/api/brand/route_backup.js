...
export async function POST(request) {
  ...
  try{
    ...
    const { name, email, phone, password, role_id, status, country_code, address, city, state, country, zip } = reqData;
    ...
    // Validate and fetch role if provided
    let roleData = null;
    if (role_id) {
      roleData = await Role.findById(role_id);
      if (!roleData) {
        return NextResponse.json({
          success: false,
    ...
    const newUser = new User({
      ...
      role: roleData ? roleData._id : null,
      role_id: role_id ? Number(role_id) : null,
      role_data: roleData ? {
        id: roleData._id,
        name: roleData.name,
        guard_name: roleData.guard_name,
        system_reserve: roleData.system_reserve
      } : null,
      address,
      city,
      state,
