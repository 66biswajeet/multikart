...
export async function GET(request) {
 ...
 try{
  ...
  if (querySortBy || querySearch || queryIds) {
    if (queryIds) {
      brands = brands.filter((brand) => queryIds.split(",").includes(brand?._id?.toString()));
    }

    // Search filter by name
    if (querySearch) {
      brands = brands.filter((brand) => brand.name.toLowerCase().includes(querySearch.toLowerCase()));
    }
