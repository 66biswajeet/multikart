...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const delete_brand_meta_image = formData.get('delete_brand_meta_image') === 'true';
    ...
    console.log("📎 Files:", { 
      ...
      brand_meta_image: brand_meta_image_file ? brand_meta_image_file.name : 'none'
    });
    console.log("🗑️ Delete flags:", { delete_brand_image, delete_brand_banner, delete_brand_meta_image });
    ...
    try {
      ...
      if (brand_image_file && brand_image_file.size > 0) {
        ...
        const tempPath = join(uploadDir, `brand-image-${Date.now()}${path.extname(brand_image_file.name)}`);
      ...
      if (brand_banner_file && brand_banner_file.size > 0) {
        ...
        const buffer = Buffer.from(bytes);
        const tempPath = join(uploadDir, `brand-banner-${Date.now()}${path.extname(brand_banner_file.name)}`);
        await writeFile(tempPath, buffer);
      ...
      if (brand_meta_image_file && brand_meta_image_file.size > 0) {
        ...
        const buffer = Buffer.from(bytes);
        const tempPath = join(uploadDir, `brand-meta-${Date.now()}${path.extname(brand_meta_image_file.name)}`);
        await writeFile(tempPath, buffer);
    ...
    const updateData = {
      ...
      ...(meta_description !== undefined && meta_description !== null && { meta_description }),
      ...(status !== undefined && status !== null && { status: Number(status) }),
      updated_at: new Date()
    };

    const updatedBrand = await Brand.findByIdAndUpdate(updateId, updateData, { new: true });
