...
const optionSchema = z.object({
  id: z.string().optional(), // Keep track of existing options
...
const UpdateVariant = () => {
  ...
  useEffect(() => {
    if (variantData) {
      reset({
        ...
        options: variantData.options.map((opt) => ({ ...opt, id: opt._id })), // Add id for field array key
      });
    }
  }, [variantData, reset]);
  ...
  // Mutation for updating the variant
  const updateVariantMutation = useMutation({
    mutationFn: (data) =>
      request({ url: `${VariantAPI}/${updateId}`, method: "PUT", data }),
    onSuccess: () => {
      ToastNotification("success", t("Variantupdatedsuccessfully"));
      queryClient.invalidateQueries(["variants"]); // Invalidate list
      queryClient.invalidateQueries(["variant", updateId]); // Invalidate detail
      router.push("/variant");
    },
    onError: (error) => {
      ToastNotification(
      ...
      <div className="card-body">
        <form onSubmit={handleSubmit(onSubmit)} className="theme-form">
          ...
          <div className="row mt-3">
            <div className="col-12">
              <FormToggle name="active" label={t("Active")} control={control} />
            </div>
          </div>
          ...
          {isOptionBased && (
            <div className="mt-4 border-top pt-4">
              ...
              {fields.map((item, index) => (
                <div
                  key={item.id} // Use item.id from field array
                  ...
                  {selectedInputType === "pattern" && (
                    <div className="col-md-4">
                      <FormInput
                        ...
                        register={register}
                        error={errors.options?.[index]?.image_url}
                      />
                    </div>
                  )}
