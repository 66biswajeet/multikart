...
const VendorProductSearch = () => {
  ...
  // Params to determine mode
  const mpid = searchParams.get("mpid");
  const editId = searchParams.get("editId"); // Get Edit ID if present
  ...
  // --- RENDER WIZARD IF CREATING OR EDITING ---
  if (mpid || editId) {
    return (
      <div className="container-fluid">
        <Row>
          <Col sm="12">
            <VendorAddProductWizard mpid={mpid} editId={editId} />
          </Col>
        </Row>
      </div>
    );
  ...
  // --- EXISTING SEARCH UI (List Page fallback) ---
  React.useEffect(() => {
    const timer = setTimeout(() => setDebouncedSearch(searchTerm), 500);
    return () => clearTimeout(timer);
  }, [searchTerm]);
  ...
  const handleSellClick = (prod) => {
    router.push(`/vendor/products/create?mpid=${prod._id}`);
  };
              ...
              {isLoading ? (
                ...
                <div className="row g-3">
                  {masterProducts?.length > 0 ? (
                    masterProducts.map((prod) => {
                      const image =
                        prod.product_thumbnail?.original_url ||
                        prod.product_galleries?.[0]?.original_url ||
                        placeHolderImage;
                      return (
                        <Col xl="3" lg="4" md="6" key={prod._id}>
                          <Card className="h-100 border">
